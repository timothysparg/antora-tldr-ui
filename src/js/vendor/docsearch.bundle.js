(() => {
	...
	activateSearch(
		require("docsearch.js/dist/cdn/docsearch.js"),
	...
	function activateSearch(docsearch, config) {
		...
		searchField.appendChild(
			Object.assign(document.createElement("div"), {
				className: "algolia-autocomplete-results",
			}),
		);
		...
		dataset.cache = false;
		dataset.source = controller.getAutocompleteSource(
			undefined,
			processQuery.bind(typeahead, controller),
		);
		...
		input.on(
			"autocomplete:cursorchanged autocomplete:cursorremoved",
			saveSearchState.bind(typeahead),
		);
		...
		menu.on(
			"mousedown.aa",
			suggestionSelector,
			onSuggestionMouseDown.bind(dropdown),
		);
		typeahead.$facetFilterInput = input
			...
			.prop(
				"checked",
				window.localStorage.getItem(SEARCH_FILTER_ACTIVE_KEY) === "true",
			);
		menu
			.find(".ds-pagination--prev")
			.on("click", paginate.bind(typeahead, -1))
	...
	function reactivateSearch(e) {
		...
		if ("type" in navigation) {
			if (navigation.type !== FORWARD_BACK_TYPE) {
				...
				this.dropdown.datasets[0].page = this.dropdown.$menu
					.find(".ds-pagination--curr")
					.data("page");
			} else if (
				window.sessionStorage.getItem("docs:restore-search-on-back") === "true"
			) {
	...
	function onResultsUpdated() {
		...
		if (
			restoring &&
			restoring.query === this.getVal() &&
			restoring.filter === this.$facetFilterInput.prop("checked")
		) {
	...
	function toggleFilter(e) {
		...
		isClosed(this)
			? this.$input.focus()
			: (this.dropdown.datasets[0].page = 0) || requery.call(this);
	}
	...
	function onSuggestionMouseDown(e) {
		var suggestion = this._getSuggestions().filter(`#${e.currentTarget.id}`);
		if (suggestion[0] === this._getCursor()[0]) return;
		this._removeCursor();
		this._setCursor(suggestion, false);
	}

	function onSuggestionSelected(e, _suggestion, _datasetNum, context) {
		if (!this.ctrlKeyDown) {
	...
	function updatePagination() {
		...
		menu
			.find(".ds-pagination--prev")
			.css("visibility", page > 0 ? "" : "hidden");
		menu
			.find(".ds-pagination--next")
			.css("visibility", result.pages > page + 1 ? "" : "hidden");
	...
	function requery(query) {
		this.$input.focus();
		query === undefined
			? (query = this.input.getInputValue())
			: this.input.setInputValue(query, true);
	...
	function processQuery(controller, query) {
		...
		if (this.$facetFilterInput.prop("checked")) {
			algoliaOptions.facetFilters = [
				this.$facetFilterInput.data("facetFilter"),
			];
		}
		var dataset = this.dropdown.datasets[0];
		var activeResult = dataset.result;
		algoliaOptions.page =
			activeResult && activeResult.query !== query
				? (dataset.page = 0)
				: dataset.page || 0;
	...
	function processQueryData(data) {
		...
		this.dropdown.datasets[0].result = {
			page: result.page,
	...
	function preserveHitOrder(hits) {
		...
		return hits.map((hit) => {
			...
			if (lvl0 !== prevLvl0) {
				lvl0Qualifiers[lvl0] =
					lvl0Qualifier == null ? (lvl0Qualifier = "") : (lvl0Qualifier += " ");
				lvl1Qualifiers = {};
			}
	...
	function readSavedSearchState() {
		try {
			...
			if (
				state &&
				(state = JSON.parse(state))._version.toString() ===
					SAVED_SEARCH_STATE_VERSION
			)
				return state;
		} catch (_e) {
